...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * 첫 번째 매개변수인 HttpServletRequest request에는 요청시 전달된 내용들이 담김(사용자가 입력한 값,
		 * 전송방식, 요청한 사용자의 ip등등
		 * 
		 * 두 번째 매개변수인 HttpServletResponse response에는 요청 처리후 응답할때 사용하는 객체
		 * 
		 * 요청을 처리하기 위해 요청시 전달된 값(사용자가 입력한 값)들을 뽑는다.
		 * request의 parameter영역 안에 존재 => key-value세트로 담겨있다.(name속성값=value값)
		 * 
		...
		// 체크박스처럼 복수개의 정보를 받을때는 배열 형태로 받아야함
		 ...
		 * 단점 : 복잡, 혹시라도 나중에 html수정해야할때 Java코드 내에서 수정이 이루어지기 때문에
		 * 		 수정된 내용을 다시 반영시키고자 한다면 서버를 재실행해야한다. 사소한 오타로 에러가 날 수 있다.
		 */
