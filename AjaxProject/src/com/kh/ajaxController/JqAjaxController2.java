...
public class JqAjaxController2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// ajax라는 녀석은 결과를 딱 한 개만 응답할 수 있음
		...
		// 버전 2) 응답데이터 여러개 보내고 싶을때 "JSON"형태로 데이터셋팅
		/*
		 * JSON(JavaScript Object Notaion : 자바스크립트의 객체 표기법)
		 *  - ajax통신시 데이터 전송에 사용되는 포맷중 하나
		 *  
		 *  - JSON처리시 사용되는 클래스 종류
		 *   => 기본적으로 자바에서 제공하지 않는다(라이브러리 필요함)
		 ...
		 * jArr.add(age); // ["40"]
		 * 
		 * // 응답할 데이터의 contextType을 설정. text/html -> 문자열
		 * response.setContentType("apllication/json; charset=UTF-8");
		 * 
		 * response.getWriter().print(jArr);
		 */
		
		// JSONObject로 객체 전달해주기
		JSONObject jobj = new JSONObject(); // {}
		jobj.put("name", name); // {name : "민재"}
		jobj.put("age", age); // {name : "민재", age : 28}
