...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 필요한 데이터를 담아서 boardUpdateForm.jsp로 포워딩시켜주기
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 필요한 데이터를 전달받고 실제 Board테이블과 Attachment테이블에 Update쿼리문을 실행
		
		// 1. 전송된 데이터 input file이 포함된 경우 enctype="multipart/form-data"로 전송했을것.
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(multi.getOriginalFileName("upfile") != null) {
				...
				// 첨부파일이 원래 등록되어있는 경우 원본파일의 파일번호와 수정된 이름을 hidden으로 넘겨받았음
				if(multi.getParameter("originFileNo") != null) {
					...
					// == at.setRefBno(Integer.parseInt(boardNo);
				}
			}
			// 하나의 트랜잭션으로 Board테이블에 update문과 Attachment테이블에 insert, update 동시에 처리해주기
			int result = new BoardService().updateBoard(b, at);
			// 항상 Board에 update문은 반드시 실행시켜줘야함
			// case1 : 새로운 첨부파일이 애초에 없는 경우(x) -> insert(x), update(x)
