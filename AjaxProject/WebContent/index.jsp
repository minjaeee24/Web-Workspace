...
<body>
	...
	<p>
		...
		* 동기식 / 비동기식 <br>
		- 동기식 : 요청처리 후 그에 해당하는 응답페이지가 돌아와야만 그 다음 작업이 가능하다<br>
		...
		요청을 보내놓고 그에 해당하는 응답이 돌아올때까지 현재 페이지에서 다른 작업을 할 수 있다<br>
		ex) 아이디 중복체크기능, 검색어 자동완성기능
		
		* 비동기식의 단점<br>
		- 현재 페이지에 지속적으로 리소스가 쌓인다 => 페이지가 느려질 수 있음<br>
		- 페이지 내 복잡도가 크게 증가함 -> 에러발생시 대처가 어려워질 수 있음<br>
		- 요청 후 들어온 응답데이터를 가지고 현재 페이지에 새로운 요소를 만들어서 뿌려줘야되는데,<br>
	...
	<button onclick="ajaxTest1();">JavaScript ajax테스트 1</button>
	
	<h3>2. 버튼 클릭시 post방식으로 서버에 데이터 전송 및 응답</h3>
	
	<button onclick="ajaxTest2();">JavaScript ajax테스트 2(POST방식)</button>
	...
	<script>
		function ajaxTest1() {
			// ajax로 서버와 통신하기
			// 1. XMLHttpRequest객체 생성하기. (form태그로 만들면 자동으로 만들어짐)
			const xhr = new XMLHttpRequest();
			
			// 2. XMLHttpRequest객체를 설정하기
			// open()함수를 이용해서 통신값을 설정
			// 매개변수1 : get/post, 2 : 요청을 보내는 주소, (생략가능)3 : 동기식, 비동기식 설정(기본값 : 동기식)
		...
		function ajaxTest2() {
			...
			xhr.onreadystatechange = function() {
				if(xhr.readyState == 4){
					if(xhr.status == 200){
						document.getElementById("target").innerHTML += "<h4>" + xhr.response + "</h4>";
					}
				}
			}
			// post방식으로 데이터전송시 파라미터 설정 방법
			// url에 데이터내용을 작성하지 않고, send함수의 매개변수로 작성해야함.
			// 전송방식도 수정해줘야함
	...
	<pre>
		...
		* 주요속성
		...
		- complete : ajax통신이 실패하든, 성공하든 무조건 실행할 함수 정의
		
		* 부수적인 속성
		- async : 서버와의 비동기 처리방식 설정 여부(기본값 true)
		- contextType : request의 데이터인코딩 방식 정의(보내는 측의 데이터 인코딩)
		- dataType : response로 오는 데이터의 자료형설정(값을 추가하지 않으면 스마트하게 판단함)
					 text : String데이터
					 xml : 트리형태의 구조
		...
		- accept : 파라미터의 타입 설정(사용자특화된 파라미터 타입 성정 가능)
		...
		- global : 기본이벤트 사용여부 => 버퍼링같이 시작과 끝을 나타낼때 선처리작업시 사용함
	...
	<h3>1. 버튼 클릭시 get방식으로 서버에 데이터전송 및 응답</h3>
	...
	<h3>2. 버튼 클릭시 post방식으로 서버에 데이터전송 및 응담</h3>
	...
	<h3>3. 서버로 데이터전송 후 조회된 객채를 응답데이터로 받기</h3>
		...
		$(function() {
			$("#xmlTest").click(() => {
				$.ajax({
					url : "books.xml",
					success : function(data) {
						let ficheader = "<tr><th>제목</th><th>저자</th></th>";
	...
	<script>
		function sendFile(){
			// 파일전송시에는 FormData라는 객체를 생성해서 파일을 추가시켜줘야함
