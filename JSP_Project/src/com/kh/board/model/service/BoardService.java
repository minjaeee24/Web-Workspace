...
public class BoardService {
	...
	public int insertBoard(Board b, Attachment at) {
		...
		int result2 = 1; // 1로 미리 선언과 동시에 초기화하는 이유 -> Attachment테이블에 Insert문이 실행되지 않을 수도 있으므로
	...
	public int updateBoard(Board b, Attachment at) {
		...
		if(at != null) {
			// 기존에 첨부파일이 있었을 경우 => update문 실행하기 위해 fileNo값이 필요함
			if(at.getFileNo() != 0) {
				result2 = new BoardDao().updateAttachment(conn, at);			
			} else { // 기존에 첨부파일 없는 경우 => insert문에는 fileNo값이 필요없어서 at객체에 fileNo값이 안 담겨있음
	...
	public ArrayList<Board> selectThumbnailBoard() {
		Connection conn = getConnection();
		
		ArrayList<Board> list = new BoardDao().selectThumbnailBoard(conn);
