...
public class ChattingServer {
	...
	public void message(Session session, String msg) { // "발송자, 수신자, 내용"
		// 첫 번째 매개변수 session객체 안에는 메세지를 보낸 클라이언트의 session객체가 담겨있음.
		...
		// 0번인덱스 : 발송자, 1번인덱스 : 수신자, 2번 인덱스 : 메세지
		...
		//   2) session클래스에서 접속한 모든 session정보를 알 수 있는 메소드를 사용 -> 버전2에서 사용할 예정
		
		// 2. 접속한 session객체를 이용해서 데이터를 전송할 수 있음
		//    session.getBasicRemote() -> 접속한 세션과 연결되는 스트림을 가져옴
		
		// 3. getBasicRemote로 가져온 객체의 sendText()라는 메소드 실행 -> 메세지를 클라이언트에 전송
