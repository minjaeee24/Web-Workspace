...
<body>
	...
	<script>
		// 웹소켓 서버에 연결
		// WebSocket객체 생성하기 192.168.30.180
		// const socket = new WebSocket("ws://ip:port/<%= request.getContextPath() %>/chatting.do")
		const socket = new WebSocket("ws://192.168.30.180:8081/<%= request.getContextPath() %>/chatting2.do");
		// https://ip:포트번호/~~
		// http -> ws:~
		// https -> wss:~
		
		// socket 설정하기
		// 1. 접속 후 실행되는 이벤트핸들러 등록
		...
		socket.onmessage = function(e) {
			console.log('메세지 수신')
			// 수신된 데이터를 받으려면 이벤트객제(e)의 data속성을 이용함
			console.log(e);
			console.log(e.data);
			// Object형태의 String데이터를 객체로 변환해주기(SDONObject)
